...
int sse_shd(data_manager* dm,int SIMD_WIDTH,int pos_index){
	...
	__m128i dh,dl,pre_dh,pre_dl;
	__m128i fh,fl,pre_fh,pre_fl;
	__m128i hbit,lbit,sh,sl,shift_count1;
	
	__m128i hamh,haml;
	__m128i ham_mask, fix_ham_mask , finalBV, pre_ham[2 * error_len + 1];
	__m128i sum;
	...
	for(int j = 0; j < read_bit_len; j ++){
		...
		dh = _mm_set1_epi32(readh[read_index * read_bit_len + j]);
		dl = _mm_set1_epi32(readl[read_index * read_bit_len + j]);	
		fh = _mm_load_si128((__m128i*)(pos_refh +  pos_index*read_bit_len + j * SIMD_WIDTH)); 
		fl = _mm_load_si128((__m128i*)(pos_refl +  pos_index*read_bit_len + j * SIMD_WIDTH)); 	
		
		//compute ham1
		 hamh = _mm_xor_si128(dh, fh);
		 haml = _mm_xor_si128(dl, fl);
		 ham_mask = _mm_or_si128(hamh, haml);
		 ...
		 finalBV=fix_ham_mask;
			...
			for(int k = 1; k <= error_len; k ++){
				...
				hbit=_mm_slli_epi32(pre_dh, BIT_PARA - k);
			    lbit=_mm_slli_epi32(pre_dl, BIT_PARA - k);
	
				sh=_mm_srli_epi32(dh, k);
				sl=_mm_srli_epi32(dl, k);
				
				sh=_mm_or_si128(hbit, sh);
				sl=_mm_or_si128(lbit, sl);
				
				hamh = _mm_xor_si128(sh, fh);
				haml = _mm_xor_si128(sl, fl);
				ham_mask = _mm_or_si128(hamh, haml);
				
				flip_false_zero(ham_mask, pre_ham[k],fix_ham_mask);
				pre_ham[k] = ham_mask;
				
				finalBV = _mm_and_si128(finalBV, fix_ham_mask);
			...
			for(int k = 1; k <= error_len; k ++){
				...
				hbit=_mm_slli_epi32(pre_fh, BIT_PARA - k);
			    lbit=_mm_slli_epi32(pre_fl, BIT_PARA - k);
				
				sh=_mm_srli_epi32(fh, k);
				sl=_mm_srli_epi32(fl, k);
				
				sh=_mm_or_si128(hbit, sh);
				sl=_mm_or_si128(lbit, sl);
				
				hamh = _mm_xor_si128(sh, dh);
				haml = _mm_xor_si128(sl, dl);
				ham_mask = _mm_or_si128(hamh, haml);
	...
	_mm_store_si128((__m128i*)(shd_scores + pos_index), sum);
...
void flip_false_zero(__m128i ham_mask, __m128i pre_ham, __m128i& fix_ham_mask){
	...
	__m128i ham_carry;
	__m128i SL1,SL2,SR1,SR2;
	__m128i TP1_AMEND1,TP2_AMEND1,TP2_AMEND2;

	//carry bit
	ham_carry=_mm_slli_epi32(pre_ham,BIT_PARA-1);
	SL1=_mm_srli_epi32(ham_mask,1);
	SL1 = _mm_or_si128(SL1, ham_carry);
	
	ham_carry=_mm_slli_epi32(pre_ham,BIT_PARA-2);
	SL2=_mm_srli_epi32(ham_mask,2);
	SL2 = _mm_or_si128(SL2, ham_carry);
	
	ham_carry=_mm_slli_epi32(pre_ham,BIT_PARA-3);
	SR1=_mm_srli_epi32(ham_mask,3);
	SR1 = _mm_or_si128(SR1, ham_carry);
	
	ham_carry=_mm_slli_epi32(pre_ham,BIT_PARA-4);
	SR2=_mm_srli_epi32(ham_mask,4);
	SR2 = _mm_or_si128(SR2, ham_carry);
	
	TP1_AMEND1 = _mm_and_si128(SL1, SR1);
	TP2_AMEND1 = _mm_and_si128(SL1, SR2);
	TP2_AMEND2 = _mm_and_si128(ham_mask, SR1);
	
	SL2 = _mm_or_si128(SL2, TP1_AMEND1);
	SL2 = _mm_or_si128(SL2, TP2_AMEND1);
	fix_ham_mask = _mm_or_si128(SL2, TP2_AMEND2);
...
void pop_count(__m128i& finalBV){
	...
	//constant varaibles
	__m128i mask[3];
	__m128i ltable,shift_count;
	__m128i haml,hamh;

	mask[0] = _mm_set1_epi32(0x0f0f0f0f);
	mask[1] = _mm_set1_epi32(0x00ff00ff);
	mask[2] = _mm_set1_epi32(0x0000ffff);
	
	//ltable = _mm_set_epi8(2, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 1, 2, 1, 1, 0);	
	 ltable=_mm_set_epi8(1, 1, 2, 1, 2, 2, 2, 1, 1, 2, 2, 1, 1, 1, 1, 0);	//xin
	//低4位和高4位shuffle并求和(不会溢出)
	haml = _mm_and_si128(finalBV, mask[0]);	
	haml = _mm_shuffle_epi8(ltable, haml);
	
	
	hamh = _mm_srli_epi32(finalBV, 4);
	hamh = _mm_and_si128(hamh, mask[0]);
	hamh = _mm_shuffle_epi8(ltable, hamh);
	
	finalBV = _mm_add_epi8(haml, hamh);

	//低8位与高8位求和
	haml = _mm_and_si128(finalBV, mask[1]);
	hamh = _mm_srli_epi32(finalBV, 8);
	hamh = _mm_and_si128(hamh, mask[1]);
	finalBV = _mm_add_epi16(haml, hamh);
	
	//低16位与高16位求和
	haml = _mm_and_si128(finalBV, mask[2]);
	hamh = _mm_srli_epi32(finalBV, 16);
	hamh = _mm_and_si128(hamh, mask[2]);
	
	finalBV = _mm_add_epi32(haml, hamh);
}
...
int find_candidates(data_manager* dm,int SIMD_WIDTH){
	...
	for(int i=read_start;i<dm->read_num;i++){
		...
		int single_pos = find_read_pos(dm, i, kmer_index,kmer_value,SSE_SIMD,pos_index);
