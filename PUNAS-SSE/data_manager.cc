...
void data_manager::init_data_manager(char* ref_file,char* read_file,float error_rate){
	...
	shd_scores=(WORD*)malloc(MAX_POS*sizeof(int));
	long long int pos_bit_size=(long long int)MAX_POS*(long long int)read_bit_len;
	pos_hbits=(WORD*)_mm_malloc(pos_bit_size*sizeof(WORD),64);
	pos_lbits=(WORD*)_mm_malloc(pos_bit_size*sizeof(WORD),64);
...
void data_manager::load_reads(char* read_file){
	...
	read_content=(char*)malloc(MAX_READ*read_len*sizeof(char));
	...
	while(read!=-1){
		...
		if(count%2==1){
			memcpy(read_content+read_num*read_len,line,read_len);
			read_num++;
		}
	}
