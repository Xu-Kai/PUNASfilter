...
int avx_shd(data_manager* dm,int SIMD_WIDTH,int pos_index){
	...
	__m512i dh,dl,pre_dh,pre_dl;
	__m512i fh,fl,pre_fh,pre_fl;
	__m512i hbit,lbit,sh,sl,shift_count1;
	
	__m512i hamh,haml;
	__m512i ham_mask, fix_ham_mask , finalBV, pre_ham[2 * error_len + 1];
	__m512i sum;
	...
	for(int j = 0; j < read_bit_len; j ++){
		...
		dh = _mm512_set1_epi32(readh[read_index * read_bit_len + j]);
		dl = _mm512_set1_epi32(readl[read_index * read_bit_len + j]);	
		fh = _mm512_load_si512((__m512i*)(pos_refh +  pos_index*read_bit_len + j * SIMD_WIDTH)); 
		fl = _mm512_load_si512((__m512i*)(pos_refl +  pos_index*read_bit_len + j * SIMD_WIDTH)); 	
		
		//compute ham1
		 hamh = _mm512_xor_si512(dh, fh);
		 haml = _mm512_xor_si512(dl, fl);
		 ham_mask = _mm512_or_si512(hamh, haml);
		 ...
		 finalBV=fix_ham_mask;
			...
			for(int k = 1; k <= error_len; k ++){
				...
				hbit=_mm512_slli_epi32(pre_dh, BIT_PARA - k);
				lbit=_mm512_slli_epi32(pre_dl, BIT_PARA - k);
	
				sh=_mm512_srli_epi32(dh, k);
				sl=_mm512_srli_epi32(dl, k);
				
				sh=_mm512_or_si512(hbit, sh);
				sl=_mm512_or_si512(lbit, sl);
				
				hamh = _mm512_xor_si512(sh, fh);
				haml = _mm512_xor_si512(sl, fl);
				ham_mask = _mm512_or_si512(hamh, haml);
				
				flip_false_zero(ham_mask, pre_ham[k],fix_ham_mask);
				pre_ham[k] = ham_mask;
				
				finalBV = _mm512_and_si512(finalBV, fix_ham_mask);
			...
			for(int k = 1; k <= error_len; k ++){
				...
				hbit=_mm512_slli_epi32(pre_fh, BIT_PARA - k);
			    lbit=_mm512_slli_epi32(pre_fl, BIT_PARA - k);
				
				sh=_mm512_srli_epi32(fh, k);
				sl=_mm512_srli_epi32(fl, k);
				
				sh=_mm512_or_si512(hbit, sh);
				sl=_mm512_or_si512(lbit, sl);
				
				hamh = _mm512_xor_si512(sh, dh);
				haml = _mm512_xor_si512(sl, dl);
				ham_mask = _mm512_or_si512(hamh, haml);
			...
			pop_count(finalBV);
			sum = _mm512_add_epi32(finalBV, sum);
			pre_dh = dh;
			pre_dl = dl;
			pre_fh = fh;
			pre_fl = fl;
	...
	_mm512_store_si512((__m512i*)(shd_scores + pos_index), sum);
	
}

void flip_false_zero(__m512i ham_mask, __m512i pre_ham, __m512i& fix_ham_mask){
	
	__m512i ham_carry;
	__m512i SL1,SL2,SR1,SR2;
	__m512i TP1_AMEND1,TP2_AMEND1,TP2_AMEND2;

	//carry bit
	ham_carry=_mm512_slli_epi32(pre_ham,BIT_PARA-1);
	SL1=_mm512_srli_epi32(ham_mask,1);
	SL1 = _mm512_or_si512(SL1, ham_carry);
	
	ham_carry=_mm512_slli_epi32(pre_ham,BIT_PARA-2);
	SL2=_mm512_srli_epi32(ham_mask,2);
	SL2 = _mm512_or_si512(SL2, ham_carry);
	
	ham_carry=_mm512_slli_epi32(pre_ham,BIT_PARA-3);
	SR1=_mm512_srli_epi32(ham_mask,3);
	SR1 = _mm512_or_si512(SR1, ham_carry);
	
	ham_carry=_mm512_slli_epi32(pre_ham,BIT_PARA-4);
	SR2=_mm512_srli_epi32(ham_mask,4);
	SR2 = _mm512_or_si512(SR2, ham_carry);
	
	TP1_AMEND1 = _mm512_and_si512(SL1, SR1);
	TP2_AMEND1 = _mm512_and_si512(SL1, SR2);
	TP2_AMEND2 = _mm512_and_si512(ham_mask, SR1);
	
	SL2 = _mm512_or_si512(SL2, TP1_AMEND1);
	SL2 = _mm512_or_si512(SL2, TP2_AMEND1);
	fix_ham_mask = _mm512_or_si512(SL2, TP2_AMEND2);
...
void pop_count(__m512i& finalBV){
	__m256i* pmmBV = (__m256i*)&finalBV;
	pop_count(pmmBV[0]);
	pop_count(pmmBV[1]);
}
